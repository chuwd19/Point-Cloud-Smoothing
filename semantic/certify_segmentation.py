...
from semantic.core import SemanticSmoothSegmentation
...
parser.add_argument('transtype', type=str, help='type of semantic transformations',
                    choices=['noise', 'points-rotation', 'points-shear', 'points-twist', 'points-taper', 'points-linear','points-noise','points-twist-rotationz'])
...
if __name__ == "__main__":
    ...
    print("idx\tcorrect\tcerified\ttime", file=f, flush=True)
    ...
    # special setting for twist x rotation
    ...
    smoothed_classifier = SemanticSmoothSegmentation(base_classifier, get_num_classes(args.dataset), transformer, canopy.shape[0])
    ...
    for i, data in enumerate(dataloader):
        ...
        point, label = data
        label = np.array(torch.squeeze(label))
        ...
        tot += len(correct)
        tot_clean += correct.sum()
        tot_good += ((radius > args.th) * correct).sum()
        time_elapsed = str(datetime.timedelta(seconds=(after_time - before_time)))
        print("{}\t{}\t{}\t{}".format(
            i, correct.sum().item(), ((radius > args.th) * correct).sum().item(), time_elapsed), file=f, flush=True)
    ...
    print((tot_clean / tot).item(), (tot_good/tot).item(), file=f, flush=True)
    f.close()
