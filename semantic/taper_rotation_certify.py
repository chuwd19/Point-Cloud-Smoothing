...
from semantic.transformers import RotationTransformer, NoiseTransformer, PointTaperRotationNoise
...
if __name__ == '__main__':
    ...
    args.alpha /= (args.slice ** 2 * args.th * args.partial * 4)
    ...
    taper_rotation = PointTaperRotationNoise(0,0,0,canopy)
    ...
    margin = math.sqrt(canopy.shape[0] * (1/4 + (1+args.th) ** 2)) / args.slice
    print(margin)
    for i, data in enumerate(dataloader):
        ...
        for j1 in range(int(2*args.slice*args.th)+1):
            if not good:
                break
            phi = j1 /args.slice - args.th
            for j2 in range(int(2 * args.slice*args.partial)+1):
                theta = j2 /args.slice - args.partial
                theta *= 180/math.pi
                now_x = taper_rotation.rotation_adder.proc(point, theta).type_as(point).to(device)
                now_x = taper_rotation.taper_adder.raw_proc(now_x, phi).type_as(point)
